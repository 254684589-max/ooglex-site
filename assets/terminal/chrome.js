...
(function (global) {
  ...
  var esc = global.OOGLEX_CORE.esc;
  ...
  /* ── 4 命令行 ───────────────────────────────────────────────────────── */
  function flash(node) {
    $$(".t-hit").forEach(function (e) { e.classList.remove("t-hit"); });
    if (!node) return;
    node.classList.add("t-hit");
  ...
  function runCommand(raw) {
    ...
    if (/^\d{1,3}$/.test(q)) {
      ...
      if (n) {
        ...
        flash(n.closest(".t-block, .t-panel, tr") || n);
        return;
      }
