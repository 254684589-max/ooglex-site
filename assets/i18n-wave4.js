...
(function () {
  ...
  var PRIVACY = {
    ...
    "「科技领袖实时动态流」优先尝试由本站后端读取公开时间线数据；免费原生时间线可能使用 X 公开 syndication，必要时使用 FxEmbed/FxTwitter 的公开接口作为服务器端回退，不需要访问者提供 X API 密钥。若这些公开源不可用，页面再回退到 X 官方网页嵌入组件；加载官方组件时，X 可能接收你访问的网页、IP 地址、浏览器类型、操作系统及 Cookie 等信息，相关处理受 X 自身隐私政策与 X for Websites 规则约束。": "Tech Leaders Live Feed first tries server-side public timeline sources. The free native timeline may use X public syndication and, when needed, the public FxEmbed/FxTwitter API as a server-side fallback, without requiring visitors to provide an X API key. If those public sources are unavailable, the page falls back to X's official web embed; when that component loads, X may receive the page visited, IP address, browser type, operating system and cookie information, subject to X's own privacy policy and X for Websites rules.",
