...
(function () {
  ...
  var PRIVACY = {
    ...
    "「全球科技领袖动态流」使用 X 官方网页嵌入组件直接展示公开时间线。加载该组件时，X 可能接收你访问的网页、IP 地址、浏览器类型、操作系统及 Cookie 等信息；相关处理受 X 自身隐私政策与 X for Websites 规则约束。若当前网络无法连接 X，本页会降级为原主页跳转入口。": "Global Tech Leaders Feed uses X's official web embed to display public timelines directly. When the component loads, X may receive the page you visited, IP address, browser type, operating system and cookie information. That processing is governed by X's own privacy policy and X for Websites rules. If X is unreachable on the current network, the page falls back to a direct profile link.",
    "「全球科技领袖动态流」可能通过 Unavatar 获取公开 X 头像。加载头像时，该第三方服务可能接收必要的网络请求信息（如 IP 地址、浏览器请求头和所请求的公开账号标识）。头像服务不可用时，本站会回退为本地生成的姓名首字母头像。": "Global Tech Leaders Feed may fetch public X avatars through Unavatar. When an avatar loads, that third-party service may receive the network request information it needs (such as IP address, browser request headers and the requested public account identifier). If the avatar service is unavailable, the site falls back to locally generated initials avatars.",
