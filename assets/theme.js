...
/* OOGLEX_REGISTRATION_PREVIEW_GATE_V1
   All application/game sections: anonymous visitors receive an approximately 10% vertical preview.
   A real Supabase Auth session unlocks the complete page. Account/privacy/home remain public. */
(function () {
  "use strict";
  ...
  function addStyles() {
    ...
    style.textContent =
      ...
      "#ooglex-registration-preview{position:absolute;left:0;right:0;z-index:2147483000;min-height:520px;display:flex;justify-content:center;align-items:flex-start;padding:72px 20px 120px;background:linear-gradient(to bottom,rgba(255,255,255,0),var(--bg,#0b0e13) 54px,var(--bg,#0b0e13) 100%);color:var(--ink,#f1f5f9);font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Microsoft YaHei','Segoe UI',sans-serif}" +
      "#ooglex-registration-preview .ogx-access-card{width:min(560px,calc(100vw - 32px));padding:26px 28px;border:1px solid var(--line,rgba(255,255,255,.14));border-radius:16px;background:var(--panel,#10141b);box-shadow:0 24px 72px rgba(0,0,0,.26);text-align:center}" +
      "#ooglex-registration-preview .ogx-access-kicker{font:700 11px/1.2 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;letter-spacing:.15em;color:var(--faint,#8b93a1)}" +
      "#ooglex-registration-preview h2{margin:10px 0 8px;font-size:24px;line-height:1.25}" +
      "#ooglex-registration-preview p{margin:0;color:var(--dim,#a8b0bc);font-size:14px;line-height:1.7}" +
      ...
      "#ooglex-registration-preview .ogx-access-secondary{border:1px solid var(--line,rgba(255,255,255,.18));color:var(--ink,#f1f5f9);background:var(--tint-1,rgba(255,255,255,.05))}";
  ...
  function applyPreview() {
    ...
    function mount() {
      ...
      var cutoff = Math.max(260, Math.floor(total * 0.10));
      var cap = cutoff + 520;
  ...
  (async function () {
    ...
    resolveReady(ok);
    try {
      document.dispatchEvent(new CustomEvent("ooglex:accessready", { detail: { authenticated: ok } }));
    } catch (_) {}
  })();
})();
