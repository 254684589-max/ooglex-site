...
| `SCRN` | 条件选股 | 基本面字段（PE / PB / ROE / 营收增速） | **源已定、管道已上线、等第一次真实运行**：`scripts/fundamentals/` + `.github/workflows/fundamentals.yml`（SEC EDGAR XBRL）。工作流只有 `workflow_dispatch`，需要 owner 先设仓库变量 `SEC_CONTACT` 并手动跑一次；`apps/companies/fundamentals.json` 一出来这一族（FA / RV / EQS）就能建。**在那之前不画空壳页** |
...
| `SCRN` 一族 | 基本面管道已上线，**等 owner 设 `SEC_CONTACT` 并手动跑一次** `fundamentals.yml`；数据出来再建 FA / RV / EQS，在那之前不画空壳 |
| 按注册表逐个加功能 | 优先级见第三节 |
