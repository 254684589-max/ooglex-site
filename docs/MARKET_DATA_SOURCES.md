...
| 品类 | 标的数（清单扩容后） | 数据来源（站内管道） | 更新频率 | 站内历史走势 |
|---|---|---|---|---|
| 商品 | 16 | 跨资产管道 Yahoo Finance 期货（含天然气、铂、钯、农产品） + 宏观雷达 EIA 库欣WTI现货 | 日频 | 有（260个交易日） |
| 指数 | 24 | 跨资产管道 Yahoo Finance（各国家/地区代表性指数 + 纳斯达克综合 `^IXIC`） | 日频 | 有（260个交易日） |
| 股票 | 40（市值前列） | 全球公司榜 Yahoo Finance | 日频 | 有（公司榜 `history.json`，260个交易日） |
| 外汇 | 12 | 跨资产管道 Yahoo Finance + 宏观雷达 FRED `DTWEXBGS` | 日频 | 有 |
| 加密 | 20 | 全球资产榜 CoinGecko（`crypto.json` 行情+日线） | 日频快照（24小时涨跌） | 有（同一次取数的日线） |
| 债券 | 14 | 宏观雷达 FRED 美债收益率曲线11个期限 + 跨资产中国国债与美债三段ETF代理 | 日频 | 有（曲线历史260点） |

> 表中条数为取数脚本清单扩容后的目标值；`apps/*/data.json` 会在对应日更任务下一次运行后达到该条数，在此之前页面按实际已发布的条数显示，不预告未发布的数据。

口径边界：

- 股票为当日价格变动，加密为过去24小时变动，债券为相对上一观测的基点（bp）变化，三者不可直接横向比较，页面逐行标注。
...
- 股票日线由公司榜任务在写完 `data.json` 之后为市值前40支各取一年补出；加密行情与日线来自资产榜任务同一次 CoinGecko 取数。两者都是主快照之后的附加步骤，失败只跳过自己并保留上次文件。
