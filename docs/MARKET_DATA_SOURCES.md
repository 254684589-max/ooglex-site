...
ETF代理会改变标的、价格和收益口径，因此卡片名称、代码和说明必须同步变化。页面以8秒Custom Element注册窗口区分`loading / registered / unavailable`；`registered`只表示组件代码已登记，不宣称报价已经渲染、新鲜或处于开市状态。TradingView组件不可用时只显示对应官方标的页链接，不得用缓存数值、演示数值或未授权API顶替；脚本晚到注册时可自动恢复。精确原标的询价包保留为未来付费升级的备用材料，并非当前上线阻塞。
