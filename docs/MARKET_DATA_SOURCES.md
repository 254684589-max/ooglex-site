...
- **纳斯达克在本页不再展示**，也没有用其他标的顶替。若要补回，可选：把纳斯达克综合 `^IXIC` 加入跨资产清单的指数类（属于新增美国基准指数点位，需所有者确认），或恢复 `QQQ` 代理卡。
...
| 指数 | 23 | 跨资产管道 Yahoo Finance（各国家/地区代表性指数） | 日频 | 有（260个交易日） |
...
4. **仍未做（需要另行决定）**：
   - 美国基准指数点位（`SPX` / `NDX` / `DJIA`）与 ICE 美元指数 `DXY` 仍按第4.3节的许可结论不在本页展示；若要展示需先取得允许公开再分发的数据许可，或明确接受以ETF代理替代。
