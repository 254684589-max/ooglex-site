...
| 品类 | 标的数 | 数据来源（站内管道） | 更新频率 | 站内历史走势 |
|---|---|---|---|---|
| 商品 | 7 | 跨资产管道 Yahoo Finance 期货 + 宏观雷达 EIA 库欣WTI现货 | 日频 | 有（260个交易日） |
| 指数 | 15 | 跨资产管道 Yahoo Finance | 日频 | 有（260个交易日） |
| 股票 | 60（市值前列） | 全球公司榜 Yahoo Finance | 日频 | 暂无 |
| 外汇 | 6 | 跨资产管道 Yahoo Finance + 宏观雷达 FRED `DTWEXBGS` | 日频 | 有 |
| 加密 | 5 | 全球资产榜 CoinGecko | 日频快照（24小时涨跌） | 暂无 |
| 债券 | 12 | 宏观雷达 FRED 美债收益率曲线11个期限 + 跨资产中国国债ETF代理 | 日频 | 有（曲线历史260点） |
...
- 股票与加密目前只有最新报价，没有可绘制的站内历史序列；抽屉里如实说明原因并给出官方来源，不用相邻标的或推断值顶替。
...
1. **股票与加密的历史序列**：沿用现有跨资产历史文件的结构（共享日期轴、滚动260点、缺观测留`null`不前向填充），为公司榜前若干标的与主流加密资产各补一份日线历史，让这两类也能画走势。
2. **商品与外汇扩容**：在跨资产管道的标的清单里增加天然气、铂、钯、农产品与更多货币对；页面无需改动即可自动出现在对应品类。
3. **指数扩容**：补齐纳斯达克综合、道指、罗素2000与VIX；注意与核心资产区四项ETF代理的口径区分，不得让指数点位与ETF代理互相冒充。
4. **搜索与自选**：品类超过百项后再加入按名称/代码搜索与自选置顶，复用现有自选清单模块。
