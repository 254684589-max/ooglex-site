...
- 逐条回退、来源待确认或估值日期缺失会使管道`degraded`，但不会等同于整批失败。
...
金融终端按三条日频管道既有的72小时时效检查`lastAttemptAt`。超过阈值后健康栏显示`STALE`和明确过期说明，并把`freshCoveragePct`标为“本轮行情”，避免把上次任务当时的覆盖率冒充当前行情新鲜度。
