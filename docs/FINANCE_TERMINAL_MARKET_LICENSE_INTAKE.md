...
> 状态：已暂停，不是当前稳定V1阻塞；仅在项目所有者未来恢复付费精确基准方案时使用
>
> 决定日期：2026-08-13
>
> 适用域名：`ooglex.com`
...
2026-08-13项目所有者已改为免费优先：当前稳定V1使用`SPY`、`QQQ`、`DIA`与`GLD`的TradingView官方免费嵌入，并明确披露为ETF代理。以下精确原标的询价范围仅作为未来付费升级预案，当前不得提交、签约或付款：

| 页面资产 | 精确目标 | 权利人/数据方 | 申请范围 | 当前状态 |
|---|---|---|---|---|
| 标普500 | `SPX`价格指数 | S&P Dow Jones Indices | 日终或延迟、公开网页展示 | **暂停 / 备用** |
| 纳斯达克100 | `NDX`价格指数 | Nasdaq | 日终或延迟、公开网页展示 | **暂停 / 备用** |
| 道琼斯工业平均指数 | `DJIA`价格指数 | S&P Dow Jones Indices | 日终或延迟、公开网页展示 | **暂停 / 备用** |
| 黄金 | LBMA Gold Price PM，USD/金衡盎司 | ICE Benchmark Administration | 伦敦午夜后延迟、公开网页展示 | **暂停 / 备用** |

统一使用场景：个人爱好者运营、无广告、无订阅、无其他商业收入，只在一个公开域名展示研究页面；不提供对外行情API、不连接交易执行、不发行或定价金融产品。

除非项目所有者明确恢复本方案，否则不得对外发送下列询价。任何报价、合同、付款、自动续费或用途扩大都必须由项目所有者逐项确认。GitHub授权、API访问权或网页可见性均不等于数据公开展示许可。
...
只有未来明确恢复付费方案后，提交前才由项目所有者在邮件或供应商表单中填写真实法定姓名、居住国家/地区和联系邮箱；这些个人信息不得提交到公开Git仓库。
