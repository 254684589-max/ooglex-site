...
### 低空镜头与连续导航（2026-09-16）

开场修复仍会被场景 LOAD、播放以及手动缩放覆盖。基础地图现在限制为最低 5000 km 的区域概览：统一包装当前 viewer 的 `setView` / `flyTo`（包含矩形目标和飞向包围球），保留目标经纬度与完成/取消回调；滚轮和触摸缩放使用 Cesium 的最小缩放距离，`preRender` 兜底处理跟踪与 `lookAt`。不改写已保存的镜头。切换到外部高清地图恢复原缩放限制，失败回退则再次拉远。

浏览器回归从本地保存的 626 m 金门大桥镜头开始，通过真实 LOAD / 播放按钮操作，检查播放完成、目标位置、每个渲染帧的最低高度和缩放后的地图。像素检查读取 WebGL 画布本身，并关闭欢迎弹窗后保存页面截图，避免把浮层的颜色误算为地图纹理。此验证不等同于大陆运营商线路实测；Overpass 等尚未接入的后端图层仍按第 5.3 节显示不可用。
