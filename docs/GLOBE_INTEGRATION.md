...
### 慢同源网络与绘图失败（2026-09-17）

此前测试只阻断第三方，站内资源始终通过本地高速服务器提供，漏掉了主站自身慢下载的情形。
Cesium 与入口脚本合计约 7.4 MB（gzip 约 2.2 MB），包装页在 45 秒时删除 iframe，
会丢弃尚有希望完成的下载。同步 Cesium 脚本也阻塞内页 HTML 解析。

包装页现在用无依赖的 `lite-globe.js` 先显示可交互的 Canvas 2D 球面投影；复用现有
NaturalEarthII 第 0 级的两张纹理（合计 26,122 字节），不新增外部数据源。它明确标注
低分辨率全球概览，不冒充高清影像或已加载的数据图层。按需绘制、限制内部画布尺寸、
隐藏时暂停；鼠标、触屏拖动、滚轮、缩放按钮和方向键均可操作。底图失败明确提示并可重试。

桌面在轻量地图完成首帧后启动完整三维；窄屏和省流量网络通过原「进入」位置的按钮加载。
12 秒未完成只说明三维仍在下载，保留 iframe、基础地球和下载进度；只有真实首帧确认后
才切入完整三维。无 WebGL 或绘图上下文丢失会保留轻量地图并给出三维重试入口。
Cesium 脚本与策略脚本按顺序 defer，基础瓦片使用独立于外部影像的超时预算。
相机最大缩放距离 50,000 km、最小 250 m，关闭缩放惯性，避免连续滚动累积到无效坐标。
高清影像/OSM 仍是可选外部来源，未更换地图供应方或主站托管。

浏览器回归新增：首屏可用后故意悬挂同源引擎超过 45 秒，检查下载不被重启且迟到资源
能恢复完整三维；360/768/1280 无 WebGL 时真实纹理与可操作性；连续极端滚轮缩放后
检查坐标有限且渲染帧继续增长；真实 WebGL 上下文丢失后的回退与重新加载。
这些模拟与海外浏览器验证不能证明大陆所有线路可达。主站域名无法连接时，
仍需针对托管线路另行定位，前端无法承诺解决域名级不可达。
