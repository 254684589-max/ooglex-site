...
1. **CSS 的 `text-transform: uppercase`。** 截图上写着 `MODELS`，DOM 里其实是
   `Models` —— 照着截图把键写成大写，运行时永远命中不了。这一类曾经漏了一整片
   （Density / Allocation / Models / Proximity / Scope / Feather / Celestial /
   Bloom / Normal / Layout / Style…）。现在精确匹配之后补一层**折叠大小写**查找
   （`FOLDED`）。它不放松上面第 1 条安全约束：依旧是整串白名单匹配，图标连字
   （`arrow_forward`）任何大小写形式都不在字典里，折叠也命不中；两个键折叠后同名
   但译文不同时两个都不进折叠表 —— 宁可漏翻，不能翻错。
2. **模板里的换行缩进。** `<span>\n  FOO\n</span>` 的文本节点带换行，而 HTML 渲染
   时空白是折叠的 —— 屏幕上看到的是 `FOO`。所以查表键统一走 `normalizeKey()`：
   去首尾空白 + 内部连续空白折成一个空格，文本节点与属性都用同一个。

### 覆盖率闸门（`scripts/globe/verify-i18n-coverage.mjs`）

「某个平时不看的面板里还有英文」以前只能靠人肉截图发现。现在它是 CI 里的确定性
断言：静态抽取构建产物 `index.html` 里作者写死的全部文案（真 `DOMParser` 解析，
不是正则；跳过图标连字、署名子树、script/style），逐条问活着的 i18n 层「查得到
中文吗」，查不到又不在 `ALLOW` 白名单里就红。

首次跑出 **212 条漏翻**（319 条文案里只有 107 条有中文），现已全部补齐；`ALLOW`
里只剩 6 条确有理由保持英文的（数据源专名 `OPEN-METEO`、视觉预设名 `CRT` 等，
每条都写了理由）。加新文案时先跑这条闸门，不要靠翻截图。

aria-label / title / placeholder / alt 现在**与可见文案同等覆盖**（闸门一起查），
不再是「主要控件优先、长尾后补」。
...
上游是桌面驾驶舱界面。它自己有移动布局（`leftPanelRail`/`rightPanelRail` 在
`max-width: 720px` 下切到 `layoutMode='mobile'`），但在中文 + 390px 下仍有三处破相，
都已在窄屏范围内修掉，**不动桌面**：

| 问题 | 实测 | 修法 |
|---|---|---|
| HUD 用桌面的 `tactical` 布局 | 390px 下 4 处文字冲出视口（`TOP SECRET // SI-TK // NOFORN`、`KH11-… OPS-…`、`常规 GLOBAL SECTOR …` 都到 x=459，超出 69px） | 窄屏自动切上游自带的 `minimal`（用应用自己的 `#hud-layout-select`，不改上游 CSS）。换成 minimal 后只剩署名链接超 4px |
| 应用的 `#title-bar` 与包装页顶栏标题重复，且横向压住旁边的圆形按钮 | — | `≤720px` 隐藏 `#title-bar`，标题由包装页负责 |
| 指令栏两个折叠面板是 `flex: 0 0 52px` 固定宽 | 「视觉预设」被截成「觉预设」 | 窄屏放宽到 96px，标签缩短为「预设」，并放开**按钮宽**与**标签 max-width** |
| 中文标签比英文短，但**字宽大得多**，上游给标签定的 `max-width` 装不下 | `.location-toolbar-label` 需 45px 只给 36.8px，「定位」被 ellipsis 吃成「定亻」（用户截图里是「定1」） | 三层一起放开（面板宽 / 按钮宽 / 标签 max-width），并把只为大写拉丁字母设计的 `letter-spacing`（约 0.16em）降到 0.02em —— 两个汉字凭空多占的 4px 正好是装不下的那几像素 |
| 语音抬头行塞三块：「AI 助手」(需 34px) + 状态 + 「STD ~$0.00」(53px)，总宽只有 73px | 「AI 助手」被压到 **2px** 宽 | 窄屏隐藏费用预估 —— 本站没配语音服务（首启提示里就写了），它是纯噪声，正好让出 53px |
| 右侧圆形电台开关 28px 见方 | 「电台」需 36px，文字溢出到圆外 | 按内容撑成一枚胶囊（`width:auto` + `padding-inline`） |
| 署名行（Cesium / Natural Earth / Data attribution）被挤出右边界 | 360px 少 34px、390px 少 4px，屏幕边上只剩半个「ion」 | 保持**单行**，标志 138px→96px、字号 10px→9px（360px 下需 382px→约 283px），并下移到读数与指令栏之间的空带。**许可要求署名可见，缩小但清晰可辨，比被裁掉一半正当** |
| HUD 底部有两处一模一样的坐标读数 | `.hud-bottom-bar`（整行）与 `.hud-corner`（「└」角标）在 390px 下上下叠着显示同样的 MGRS + 经纬度，还占掉署名行唯一的空带 | 窄屏隐藏角标那份，保留可读性更好的整行 |

包装页顶栏在 ≤420px 时一共要放五个控件（返回 / 标题 / 中EN / 数据来源 / 主题），
另有一条规则压间距与字号，**不隐藏任何功能**。

实测（360px / 390px / 430px）：0 处残留英文、0 处文字冲出视口、**0 处文字被裁**；
桌面 1280px 仍是 `tactical` 且 `#title-bar` 正常显示，互不影响。

> **「被裁」要同时看两种裁法。** 第一版检测只查了元素自己溢出
> （`scrollWidth > clientWidth`），漏掉了「被某一层祖先的 `overflow: hidden` 切掉」
> —— 后者元素自己的 `scrollWidth` 完全正常。也不能只看叶子节点：真正被裁的往往是
> 「图标 + 文字」那一层（它有子元素）。两条都栽过，闸门现在两种都查。

> **缓存提醒**：微信内置浏览器等 WebView 对 HTML 缓存很顽固，改完可能仍看到旧版。
> 判断方法：旧版顶栏下面**多一条**「全球视角 / 切换高清影像」状态栏；新版只有一条
> 顶栏。遇到时强制刷新即可。

## 3.2b 页面结构：单栏 + 单层 iframe（刻意如此）
...
### 基础底图到底能清晰到什么高度（2026-09-17，实测）

判断「清不清晰」不能用**颜色数** —— 糊掉的渐变同样有上百种颜色（500 km 高度一片糊
也有 88 色，第一版就是这么误判成「有细节」的）。改用**边缘能量**：取画面中心
200×200 区域、相邻像素亮度差（0.299R+0.587G+0.114B）的均值，越大越锐。

量的必须是**屏幕上真正的像素**（对页面截图取样），不是 canvas 的像素缓冲 ——
后者量不出 dpr 放大造成的糊。同 dpr 下 before/after 可直接比；跨 dpr 要先缩回
CSS 像素再比。

390px / dpr=3（设备像素边缘能量 | 缩回 CSS 像素后）：

| 相机高度 | 设备像素 | CSS 归一 | 颜色数 | 观感 |
|---|---|---|---|---|
| 22,000 km | 1.42 | 3.35 | 831 | 清晰 |
| 8,000 km | 2.01 | 4.91 | 666 | 清晰 |
| 3,000 km | 0.69 | 1.86 | 338 | 偏糊 |
| 1,500 km | 0.37 | 1.02 | 272 | 偏糊 |
| 1,000 km | 0.25 | 0.66 | 193 | 模糊 |
| 700 km | 0.18 | 0.49 | 141 | 模糊 |
| 500 km（下限） | 0.14 | 0.38 | 110 | 模糊，但**不是纯色** |
| 468 m（无下限时） | — | — | **1** | 一片纯绿，这才是「坏了」 |

> 22,000 km 那行的设备像素读数（1.42）比 8,000 km（2.01）低，不是它更糊：那个高度
> 地球只占取样区的一部分，剩下是星空背景，把均值拉低了。**看同一列的相邻两行才有
> 意义**，跨行绝对值不可比。

**结论：8,000 km 以下就没有清晰可言。** 要做到「永不发糊」，下限得抬到 8,000 公里
往上 —— 那等于禁止放大，比发糊糟得多。所以下限保持 500 km：它的职责只有一个，
**挡住一片纯色**（颜色数 1），而不是保证锐度。

> 相应地，两条「不能是一片纯色」的闸门**刻意只判有无内容，不判锐度**；锐度断言只
> 放在概览视角（`verifyVisibleEarth`，只在 1280px 跑，实测 2.0–3.2，闸门取 >1，
> 离实测下沿有一倍余量），那里本地底图确实给得出细节，掉到 0 附近就说明底图掉了
> 或渲染坏了。

### 试过但**不要**再试：提高渲染分辨率（2026-09-17，负面结论）

手机上地球发糊，第一反应是 Cesium 的 `useBrowserRecommendedResolution` 默认 `true`，
像素缓冲按 **CSS 像素**分配：

```
canvas.width = clientWidth × (useBrowserRecommendedResolution ? 1 : dpr) × resolutionScale
```

实测 390×844 / dpr=3 时画布确实只有 390×800，再由浏览器放大三倍贴到屏幕上。
把 `resolutionScale` 提到 `min(dpr, 2)`（画布 780×1600）之后：

* 设备像素边缘能量 22,000 km **1.42 → 1.46**、3,000 km 0.69 → 0.66、500 km 0.14 → 0.14
  —— 都在噪声范围内；
* 同一位置的截图**逐像素看不出差别**（本地底图、卫星标签 + 轨道线两种场景各比过一次）。

原因：瓶颈是**影像本身**（NaturalEarthII，`maximumLevel: 2`，全球 1024×512），
不是渲染分辨率；而这个应用的标签、引线、读数全是 **DOM 元素**，浏览器本来就按物理
像素渲染，不经过 WebGL 画布。于是代价（dpr=3 时四倍片元工作量，手机 GPU 上是实打实
的发热与掉帧）换不到任何可见收益。**已撤回，不要再加。**

真要解决发糊只有一条路：换更高分辨率的底图。本地方向需要把公有领域影像
（Natural Earth II 1:10m，16200×8100）切成 0–4 或 0–5 级瓦片（约 10 MB / 40 MB）
入库；本沙箱的出网策略拦住了 naturalearthdata.com 与 NASA，没有做。
远端方向是应用自带的高清影像（Esri），触到下限时已会自动试一次。
