...
- 每个节点带 `stageBasis` 字段标明口径来源：`sector-initial`（板块级初步）／
  `sic-refined`（SIC 行业码细化）／`edge-derived`（真实上下游边反推）；
...
| 层 | 内容 | 验收标准 | 状态 |
|---|---|---|---|
| 0 | 节点表 + 价值链阶段初步口径 | 495 节点齐全，`stageBasis` 逐节点标注，零新增数据源 | **建设中** |
