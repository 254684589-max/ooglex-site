...
5. 辅助资格在四源刷新后使用最终页面重新生成代理趋势和运维评估；它只验证同一页面在刷新后的组件回退契约，不把CNN、OFR、经济日历或新闻的运行次数计入代理历史，也不成为质量趋势的远端历史源。
...
- 进入公开Beta评审前，四条工作流必须分别连续成功至少3个日更周期且门禁无`BLOCKED`；稳定V1还需7个周期并消除市场基准演示数据。
