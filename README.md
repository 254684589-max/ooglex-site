...
│   ├── university-rankings/ # 🎓 全球大学排名 300 强（QS · THE · ARWU · U.S. News 四榜合一·平均位次综合排序，每日更新）
...
- **数据每日自动更新**：`.github/workflows/university_rankings.yml` 每日由 `scheduler.yml` 统一触发（北京时间 07:00 前）跑
  `scripts/university-rankings/build_universities.py`，抓取 QS 榜单 AJAX 端点、THE 页面内嵌整表 JSON、ARWU 与
  U.S. News 官方接口（均免密钥），写入 `apps/university-rankings/data.json` 提交回仓库；前端 `fetch` 即时渲染。
- **强容错**：四源相互独立，有效源不足 2 个则保留上次 `data.json` 不覆盖。内置一份取自四榜近一期公开位次整理的
  **SEED 上线快照**（页面标注「近似值」），保证首跑前即有可用数据；首次在 GitHub Actions 成功抓取后替换为四榜实时数据。**仅供参考，不构成升学建议。**
