...
├── games/                  # 网页小游戏
│   ├── hub/                # 🎮 游戏中心（游戏合集入口）
│   ├── working-life/       # 🌃 打工 WORKING LIFE（原创 Godot 4 3D 赛博朋克都市人生模拟 · godot/ 为工程源码，play/ 为网页导出）
...
`apps/movies/`（数据中心成员）以**海报墙**形态展示 **高分电影 Top 250**、**全球最新上映** 与 **公版经典**
三张榜单，含排名、评分、票数、海报与中文片名；并带一段 **Web Audio 生成的影院氛围背景音乐**（🎵 开关）。

- **点海报即播（全部合法渠道）**：点击海报弹窗播放——普通影片内嵌 **YouTube 官方预告片**
  （youtube-nocookie 隐私增强域名），弹窗内同时列出各地区**正版观看渠道**（TMDB × JustWatch，HK/TW/US）；
  「公版经典」为**美国公有领域**影片，完整正片内嵌 **Internet Archive** 官方播放器。
  本站不存储、不提供任何受版权保护影片的正片资源。
- **数据每日自动更新**：`.github/workflows/movies.yml` 每日由 `scheduler.yml` 统一触发（北京时间 07:00 前）跑 `scripts/movies/build_movies.py`，
  ...
  （最新上映），以 `language=zh-CN` 取中文片名与海报，并逐片补取 `videos`（预告片 key）与
  `watch/providers`（观看渠道），写入 `apps/movies/data.json`；公版片源构建时逐条经 Internet Archive
  metadata API 校验，失效自动按片名搜索高下载量条目替换，找不到则剔除。
