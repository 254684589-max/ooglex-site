...
def main() -> None:
    ...
    # 17,000：行情板多读一份各国主权债收益率（apps/bonds/data.json），并把上游与本地
    # 证据两个来源计数改为按路径登记表现算，不再手写——省下的是每次扩容漏改魔数的代价。
    require(LOADER.stat().st_size <= 17_000, "金融终端分区加载模块超过17KB性能预算")
    ...
    # 38,500：债券品类接进各国主权债收益率并分了二级组，多出一张分组表、一张逐代码
    # 登记表（美债 11 个期限 + 11 只债券基金）与主权债那段行构造。它比别的品类多花的
    # 篇幅都在注释上：这一类里同时摆着收益率百分数与美元计价的基金份额价格，
    # 为什么必须分开摆、为什么涨跌用基点而不是百分比，都写在代码旁边。
    # 42,000：外汇品类接进 80 个货币对并按地区分了二级组，多出一张分组表、一张
    # 「货币 → 地区」表与一个解析函数。登记的是货币而不是货币对：地区属于货币，
    # 八十多个对里只出现五十来种货币，再加一个 USDxxx 对时这张表不必跟着改。
    # 43,000：公司行的过滤条件从「只要实时行情」改成「排除估值/待确认/取不到」，
    # 多出一张排除模式表与说明它为什么不再等同于 mode === "market" 的两行注释。
    require(BOARD_DATA_MODULE.stat().st_size <= 43_000,
            "按需加载的品类行情板数据层超过43KB性能预算")
    ...
    # 32,500：债券品类接进各国主权债后多出两段——一段按「日频/月频分桶」取主权债历史
    # （与商品现货同一种结构），一段把四个区间涨跌列在收益率行上改算基点差而不是相对
    # 涨幅。后者顺带修好了美债曲线那 11 行既有的同一处误导：收益率从 3.05% 到 2.97%
    # 是「下行8个基点」，写成 −2.62% 会被读成价格跌了 2.6%。
    require(BOARD_VIEW_MODULE.stat().st_size <= 32_500,
            "按需加载的品类行情板视图超过32.5KB性能预算")
    ...
    # 38,500：新增各国主权债这一类标的的读取器——它与商品现货共用「日频/月频分桶」
    # 那套历史结构，但涨跌是基点、来源分 FRED 与 ECB 两处，因此单独一个读取器。
    require(QUOTE_MODULE.stat().st_size <= 38_500, "行情详情页脚本超过38.5KB性能预算")
    ...
    # 15,000：债券品类接进各国主权债后，导语里必须写清三件事——这一类里同时摆着
    # 收益率与基金份额价格两种量纲、各国十年期几乎全是月频、以及哪些国家在免费源上
    # 根本没有序列。这三条都是诚实边界，不能为了省字节删掉。
    # 导语本身已经偏长（这一版是压缩过的），是否折叠成「展开阅读」另议。
