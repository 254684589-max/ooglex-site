...
assert(ytdRanking.leaders.concat(ytdRanking.laggards).every((asset) => !asset.stale && !as…9946 tokens truncated…一致")
    ...
    require(GLOBE_MODULE.stat().st_size <= 8_200, "金融终端地球动画模块超过8.2KB性能预算")
    # 5,800：在原正射采样上增加四邻域亮点对比，只增强纹理中真实存在的城市灯光，
    # 不生成随机灯点；初次渲染仍受520px纹理上限约束，旋转缓存策略不变。
    ...
    require('from "./finance-terminal-globe.mjs"' in command_center_module
            ...
            and "drawConnections" in globe_module
