...
EXACT: dict[int, tuple[str, str]] = {
    ...
    # SIC 3751 的官方名称是 "Motorcycles, Bicycles, and Parts"——**一个码同时
    # 覆盖整机与零部件**，光看码分不出来。不写这一条的话它会落到下面
    # (3700,3799) 那个「运输设备零部件」兜底区间，于是电动两轮车整车厂
    # （EZGO、LOBO）被放进零部件环节，页面上是明显错的。
    # 判为整机，理由是报这个码的发行人以整车制造为主，零部件厂通常报 3714
    # 或 3799。**这是取多数读法，不是这个码没有歧义**——真正的解法是让
    # sic_stages 支持候选集（现有歧义机制只接了板块级），记在台账里。
    3751: (STAGE_FINISHED, "摩托车与自行车整机"),
