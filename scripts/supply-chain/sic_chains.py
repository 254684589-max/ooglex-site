...
#     读者反而看不出传导顺序。
# 二、**横跨全链的使能链不参与分层，但该有的入边照连。**
#     金融、商业服务、物流、地产**向外**供给几乎所有链，逐条连它们的**出边**
#     会画出 100 多条没有信息量的线，所以出边不连、也不给层号（它们横跨所有层，
#     排进任何一层都是错的）。
#
#     **但这不等于它们没有入边。** 石油给物流的是运输燃料、汽车给物流的是
#     公路运力、软件给金融的是支付清算系统——这些是真的、具体的投入，
#     连出来有信息量，就该连。
#
#     这两件事曾经被写成一句「不逐条连线」，于是数据里明明有 5 条入边，
#     文案却说「未逐条连线」，路径视图还把物流从「石油的下游」里静默丢掉了。
#     **「不参与分层」和「不逐条连」是两个不同的判断，不能合并成一句话。**
#
...
# 横跨全部产业链的使能链：**出边不逐条连、也不给层号**，但入边照连（见上方规矩二）。
# 措辞要分清这两件事——写成「不逐条连线」会与数据里真实存在的入边打架。
CROSS_CUTTING: dict[str, str] = {
    "financial-services": "资金、保险与支付横跨全部产业链，其出向不逐条连线",
    "real-estate": "厂房、仓储与办公物业横跨全部产业链，其出向不逐条连线",
    "business-services": "咨询、人力、检测与认证横跨全部产业链，其出向不逐条连线",
    "logistics-transport": "运输与仓储衔接每一段实物流转，其出向不逐条连线",
}
