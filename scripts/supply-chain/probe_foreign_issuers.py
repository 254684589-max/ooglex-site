...
# 6-K 是它们的临时报告，单独统计只作交叉验证——年报窗口错开的能被它兜住。
...
def main() -> int:
    ...
    try:
        payload = _get_json("https://www.sec.gov/files/company_tickers_exchange.json")
    ...
    for cik in sample:
        try:
            meta = _get_json(f"https://data.sec.gov/submissions/CIK{cik:010d}.json")
            time.sleep(GAP)
