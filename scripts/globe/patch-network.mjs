...
export function patchHtml(input, base) {
  ...
  // Preserve execution order without blocking HTML parsing behind the engine.
  html = html.replace(/<script (?:defer )?src="([^"\n]*(?:cesium\/Cesium\.js|network-policy\.js\?v=\d+))"><\/script>/g,
