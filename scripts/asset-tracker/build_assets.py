...
ASSETS = [
    ...
    # 同日实测撤下 NYMEX丙烷（B0=F）：首轮真实取数返回「行情数据点不足」，该代码在数据源上
    # 没有可用的免费日线。它只让管道变成 degraded，页面上一行都不会多，因此不留在清单里。
    ...
    # —— 2026-08-30 扩容：外汇从 22 条扩到 81 条（80 个货币对 + 美元指数），并在行情板按地区分组 ——
    ...
    #
    # 实跑（Asset Tracker #124）后撤下的一条：离岸人民币 USDCNH=X / CNH=X 两个候选代码
    # 都没有返回有效行情，因此不登记。也不拿在岸人民币（USDCNY=X，已在上面单列）顶替：
    # 在岸与离岸是两个市场、两个价，用一个冒充另一个就是把假事实写进页面。
    # 其余 81 个货币对本轮全部取到实时行情。
