...
def report_with_cycles(cycles: int, *, latest: str = "success") -> dict:
    dates = [f"2026-08-{11 - offset:02d}" for offset in range(cycles)]
    checks = []
...
def test_repository_snapshot() -> None:
    ...
    validate_snapshot(value)
    require(value["targetBranch"] == BRANCH, "仓库快照必须属于当前完整开发分支")
