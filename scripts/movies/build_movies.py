...
# 公版经典：均为美国公有领域影片，ia 为 Internet Archive 条目标识（已逐条校验过可播）。
# 构建时逐条复核，标识失效会按片名搜索自动替换（见 classics_list），仍找不到则剔除。
CLASSICS = [
...
def classics_list():
    ...
    items = []
    for c in CLASSICS:
        ident = c["ia"] if ia_meta(c["ia"])[0] else ia_find(c["orig"], c["year"])
        if not ident:
