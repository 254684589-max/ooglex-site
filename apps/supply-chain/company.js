...
(function () {
  ...
  var TIERS = [
    { id: "t1", name: "一级供应商", role: "直接供货", real: false,
      ...
      gap: "没有免费数据源。七种办法都已实测否决——前四条是 SEC，后三条在 SEC"
         + "之外："
         + "①客户集中度披露（ASC 280）要求披露占比但不要求写客户名；"
         + "②EDGAR 全文反查按词频排序，提及某公司最多的往往是起诉它的人；"
         + "③10-K 附件 21 子公司清单列的是法律实体，与冶炼厂设施名严格同名 0 条；"
         + "④10-K 附件 10 材料合同（实测 80 家）——找到附件的 61 家里，标题属于"
         + "供货／制造／分销类的只有 10 份，而其中 4 份真合同的对方名称按保密"
         + "处理规则可以涂掉；"
         + "⑤USAspending 联邦分包（实测 100 家）——只有 5 家能锚定到联邦收件方，"
         + "解出具名分包关系 2 家，且语义是「同一份联邦合同的总包与分包」，"
         + "不是「谁向谁供货」；"
         + "⑥FCC 设备认证（实测 119 家）——公开数据集是持证人登记表，"
         + "一行只有一个当事方加地址，表的结构里就没有「谁替谁造」这件事；"
         + "⑦政府召回公告（实测 200 条）——召回文书写的是危害与补救，"
         + "不交代供应来源；即使提到供应商也只写「the supplier」而不写是谁，"
         + "200 条里只解出 3 条，真正可用 1 至 2 条。"
  ...
  var T3_EMPTY = {
    ...
    "index-says-filed": {
      chip: "索引有、本轮未读到",
      // 这一档是生产流水线连死第三轮逼出来的：EDGAR 季度索引标了这家报过
      // Form SD，而本轮抽取器没读到那份申报。它**不是「未见申报」**（索引说
      // 报过），也**不是「有申报无名单」**（我们没打开那份申报，不知道里面
      // 有没有名单）。如实说成我们这边的缺口，并且不计入「未见申报」——
      // 那会把它从覆盖率分母里排除，是在粉饰覆盖率。
      text: "EDGAR 的季度申报索引里，这家公司标记为提交过 Form SD，"
          + "但本轮抽取器没有读到那份申报。"
          + "所以这一栏空着是我们这边的缺口，不是披露的形态——"
          + "它既不算「未见申报」，也不算「申报里没名单」，"
          + "因为我们没有打开过那份文件。"
          + "点下面的链接可以直接到 EDGAR 核验它报过什么。"
    },
    "failed": {
      chip: "抽取缺陷",
  ...
  var SD_LABEL = {
    ...
    "index-says-filed": "索引有申报、本轮未读到（缺口）",
    "failed": "扫到名单但未写成（缺陷）",
    "": "未测"
  };
  ...
  function renderSide() {
    ...
    if (pv && (pv.peers || []).length) {
      ...
      // **重叠为什么这么大，得就地说。** 只说「不表示业务往来」是不够的：
      // 读者看到「重叠 349 家」仍会当成两家供应链高度耦合的强信号。实测的
      // 解释是这些名单本身高度雷同——中位公司名单里每一家厂都被别家也列了。
      // 缺这句话，这一屏就在放任一个错误推论。
      var sim = ((d.listSimilarity || {}).byCompany || {})[n.symbol];
      if (sim) {
        var same = Math.round((sim.jaccard || 0) * 100);
        // 单开一个类名。契约里那条「写明重叠不表示业务往来」的断言取的是
        // 第一个 .pwarn——把这段也叫 pwarn 会把它挤到后面，断言当场读到
        // 另一段文字。**新加的话不能顶掉旧的界线**，两段各守各的。
        pbox.appendChild(el("p", "psim",
          "重叠大是常态，不是信号：这些名单在很大程度上是同一份 RMI 合规冶炼厂"
          + "名录的再现。本公司名单与最相似的一家（" + (sim.closest || "—")
          + "）重合度 " + same + "%，"
          + (sim.uniqueShare > 0
             ? "名单里只有 " + Math.round(sim.uniqueShare * 100)
               + "% 是别家都没列过的。"
             : "名单里每一家冶炼厂都被别的申报人也列了。")
          + "所以要看的不是重叠多大，而是独有的那部分。"));
      }
