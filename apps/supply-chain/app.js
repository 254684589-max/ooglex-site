  ...
  function renderNotice(d) {
    ...
    setText($("notice-edges"), edges === 0
      ...
      : "已收录 " + edges + " 条带出处的关系" + scope
        ...
        + "间接、不含份额，不是直接供货关系。"
        // 原来写「一级与二级供应商仍无数据源」。「仍」读起来像「还没做到」，
        // 而这四条路是逐条实测否决的——说成待办就把实测结论降格成了进度。
        + "一级与二级供应商 0 条：七种办法已逐条实测否决——SEC 四条（客户集中度、"
        + "全文反查、附件 21 子公司、附件 10 材料合同）加联邦采购分包、"
        + "FCC 设备认证、政府召回公告，这不是还没做。"
