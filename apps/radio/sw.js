...
self.addEventListener('fetch', (event) => {
  ...
  // 跨域一律放行（电台音频流 / radio-browser API / 外站台标），不缓存、不拦截
  if (url.origin !== self.location.origin) return;

  // 只处理本应用作用域内的资源
  if (!url.pathname.startsWith('/apps/radio/')) return;
  ...
  // 其余同源静态资源：stale-while-revalidate（忽略查询串，兼容 stations.json?t=… 的缓存穿透）
  event.respondWith(
