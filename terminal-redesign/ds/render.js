/* ===========================================================================
   OOGLEX 终端渲染器 · Terminal Renderers
   ---------------------------------------------------------------------------
   把 feed.js 的模型渲染进 ds.css 的组件里。三份草案（指挥台 / 监控墙 / 研究台）
   共用同一套渲染器，只是布局与取用的模块不同 —— 这样三版的数字口径必然一致，
   改一处口径三版同时生效。

   每个渲染器都只依赖：一个模型 + 一组目标选择器，不关心页面布局。
   =========================================================================== */
(function (global) {
  "use strict";
  var F = global.OOGLEX_FEED, fmt = F.fmt, isNum = F.isNum;
  ...
  function srcLine(s, extra) {
    ...
    return "来源 " + (s.source || "—") + " · AS OF " + (s.asOf || "—") + " · " +
      (s.cadence || s.frequency || "—") + " · 状态 " + fmt.statusZh(s.status) + (extra ? " · " + extra : "");
  }
  ...
  function topStatus(M) {
    ...
    var un = $("[data-unavail]");
    if (un) un.textContent = F.UNAVAILABLE_NOTE;
  }
