#!/usr/bin/env python3
"""把 Godot 网页导出结果整理进 games/construction-worker/play/，并给文件名加内容哈希。
...
文件名：cw-<哈希>.wasm、cw-<哈希>.pck ……引擎或游戏数据一变，文件名就变，
回访玩家不会拿到「新页面 + 旧游戏包」的组合。

用法（一般由 build_web.sh 调用）：
    python3 stamp_web_build.py <godot 导出目录> <games/construction-worker/play>
"""
...
def main() -> int:
    ...
    new = f"cw-{sha8(wasm + pck + js)}"

    dst.mkdir(parents=True, exist_ok=True)
    for old in dst.iterdir():
        if old.is_file() and (old.name.startswith("cw-") or old.name == "index.html"):
            old.unlink()
